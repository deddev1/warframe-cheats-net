/**
 * Attach warframecheats.net to Worker islecheat and create DNS CNAMEs.
...
const DOMAINS = ['warframecheats.net', 'www.warframecheats.net'];
...
async function ensureCname(name, content) {
	const fqdn = name.includes('.') ? name : `${name}.warframecheats.net`;
	const existing = await listDns();
	const hit = existing.find(
		(r) => r.name === fqdn || r.name === name || (name === '@' && r.name === 'warframecheats.net'),
	);
	if (hit) {
	...
	const recordName = name === 'warframecheats.net' || name === '@' ? 'warframecheats.net' : name;
...
async function main() {
	...
	await ensureCname('warframecheats.net', TARGET);
